...
export class IndustriasTableComponent {
  ...
  setMaterialPrima(industria: IndustryType, index: number, good: string) {
    this.store.dispatch(setMateriaPrima({ industria: industria.name, index, good }));
  }

  setMaterialPrimaPerWeek(industria: IndustryType, index: number, nivel: number, perWeek: number) {
    this.store.dispatch(setMateriaPrimaPerWeek({ industria: industria.name, index, nivel, perWeek }));
  }

  setProduct(industria: IndustryType, index: number, good: string) {
    this.store.dispatch(setProduct({ industria: industria.name, index, good }));
  }

  setProductosPerWeek(industria: IndustryType, index: number, nivel: number, perWeek: number) {
